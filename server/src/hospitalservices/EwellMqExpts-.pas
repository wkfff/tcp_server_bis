...
function TMQClass.QueryByParam(const AParam: UnicodeString): Integer;
var
  iReturn: Integer;
  pSecId: PAnsiChar;
  pMsgId: PAnsiChar;
...
begin
  ...
  try
    try
      ...
      pPutMsg := ConvertStringToAnsiChar(System.UTF8Encode(AParam));
      iReturn := PutMsgMQ(ConvertStringToAnsiChar(ServiceId), @pMsgId[0], pPutMsg, @pErrorMsg[0]);
      ...
      iReturn := GetMsgMQ(ConvertStringToAnsiChar(ServiceId), 1000 * 10, @pMsgId[0], pGetMsg, @pErrorMsg[0]);
      if iReturn <> 1 then
      begin
      ...
      if Active then
        DisConnect;
//      CnDebugger.LogMsgWithTag(pGetMsg, 'MQGet');
